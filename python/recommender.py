...
def suggest_best_manufacturer(input_data):
    try:
        ...
        if len(df) == 1:
            ...
            df['cost_score'] = 1 - (df['total_cost'] - df['total_cost'].min()) / (df['total_cost'].max() - df['total_cost'].min() + 1e-6)
            df['dist_score'] = 1 - (df['distance'] - df['distance'].min()) / (df['distance'].max() - df['distance'].min() + 1e-6)
        ...
        return {"error": f"Internal error in recommendation: {str(e)}"}